...
public final class GameObjectDefinition {
    ...
    public static GameObjectDefinition forId(int i) {
        if (i >= GameSettings.OSRS_OBJECTS_OFFSET) {
            for (int j = 0; j < 20; j++)
                if (cacheOSRS[j].id == i)
                    return cacheOSRS[j];

            cacheIndexOSRS = (cacheIndexOSRS + 1) % 20;
            GameObjectDefinition ObjectDef = cacheOSRS[cacheIndexOSRS];

            try {
                dataBufferOSRS.currentOffset = streamIndicesOSRS[i - GameSettings.OSRS_OBJECTS_OFFSET];
            } catch (Exception e) {
                GameServer.getLogger().log(Level.SEVERE, "ruh roh", e);
            }
            ...
            ObjectDef.readValues(dataBufferOSRS);

            int id = i - 300000;
            // OSRS unwalkable definitions
    ...
    public static void init() {
        ...
        dataBufferOSRS = new ByteStreamExt(getBufferOSRS("loc.dat"));
        ByteStreamExt streamIdxOSRS = new ByteStreamExt(getBufferOSRS("loc.idx"));
        ...
        cacheOSRS = new GameObjectDefinition[20];

        for (int k = 0; k < 20; k++) {
            cache[k] = new GameObjectDefinition();
            cacheOSRS[k] = new GameObjectDefinition();
        }
    ...
    public static byte[] getBuffer(String s) {
        try {
            java.io.File f = new java.io.File(System.getProperty("user.dir") + "/data/clipping/objects/" + s);
            if (!f.exists())
                return null;
    ...
    public static byte[] getBufferOSRS(String s) {
        try {
            java.io.File f = new java.io.File(System.getProperty("user.dir") + "/data/clipping/osrs/objects/" + s);
            if (!f.exists())
                return null;
            byte[] buffer = new byte[(int) f.length()];
            java.io.DataInputStream dis = new java.io.DataInputStream(new java.io.FileInputStream(f));
    ...
    private void readValues(ByteStreamExt buffer) {
        boolean osrs = buffer == dataBufferOSRS;
        int i = -1;
        label0:
        do {
            int opcode;
    ...
    public static GameObjectDefinition[] cacheOSRS;
    public int anInt783;
