...
public class ItemsKeptOnDeath {
    ...
    public static void sendInterfaceData(Player player) {
        ...
        if (player.getSkullTimer() > 0)
            infoToFill[0] = "(0) @or2@Skulled";
        else
            infoToFill[0] = "(3) @gre@Not Skulled";

        if (keepExtraItem)
            infoToFill[1] = "(1) @gre@Protect item [on]";
        else
            infoToFill[1] = "(0) @or2@Protect item [off]";

        if (player.wildHoarder > 0)
            infoToFill[2] = "(1) @gre@Hoarder [unlocked]";
        else
            infoToFill[2] = "(0) @or2@Hoarder [locked]";

        infoToFill[3] = "";


        infoToFill[4] = "@or2@All untradeable items";
        infoToFill[5] = "@or2@are automatically kept.";
    ...
    public static ArrayList<Item> getItemsToKeep(Player player) {
        ...
        for (Item item : Misc.concat(player.getInventory().getItems(), player.getEquipment().getItems())) {
            ...
            if (item == null || item.getId() <= 0 || item.getAmount() <= 0 || !item.tradeable()) {
                ...
                if (item.isStatuette(player, item)) {
                    coins = item.getDefinition().getValue() * 2500 * item.getAmount();
    ...
    public static int getAmountToKeep(Player player) {
        boolean keepExtraItem = PrayerHandler.isActivated(player, PrayerHandler.PROTECT_ITEM) || CurseHandler.isActivated(player, CurseHandler.PROTECT_ITEM);
        return (player.getSkullTimer() > 0 ? 0 : 3) + (keepExtraItem ? 1 : 0) + (player.wildHoarder > 0 && PerkHandler.canUseWildernessPerks(player) ? 1 : 0);
    }
