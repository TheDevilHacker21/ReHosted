    ...
    public static boolean depositMoney(Player plr, int amount) {
        ...
        if (validateAmount(plr, amount)) {
            ...
            if (addedMoney > Long.MAX_VALUE) {
                ...
                if (plr.getInventory().getAmount(995) >= canStore) {
                    ...
                    if (plr.personalFilterPouch) {
                        plr.getPacketSender().sendMessage("You've added " + canStore + " coins to your money pouch.");
                    }
                    return true;
                } else {
                    plr.getPacketSender().sendMessage("Nice attempt to glitch the money pouch.");
                    //World.sendMessage("A player has attempted to glitch the money pouch.");
                    return false;
                }
            } else {
                if (plr.getInventory().getAmount(995) >= amount) {
