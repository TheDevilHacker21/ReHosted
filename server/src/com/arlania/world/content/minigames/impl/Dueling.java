...
public class Dueling {
    ...
    public void challengePlayer(Player playerToDuel) {
        ...
        if (player.getGameMode() == GameMode.ULTIMATE_IRONMAN) {
            player.getPacketSender().sendMessage("Ultimate-Ironman-players are not allowed to stake.");
            return;
        }
    ...
    public void stakeItem(int itemId, int amount, int slot) {
        ...
        if (playerToDuel == null)
            return;
        if (player.getStaffRights() != StaffRights.DEVELOPER && playerToDuel.getStaffRights() != StaffRights.DEVELOPER) {
    ...
    public void selectRule(DuelRule duelRule) {
        ...
        boolean slotOccupied = duelRule.getEquipmentSlot() > 0 && (player.getEquipment().getItems()[duelRule.getEquipmentSlot()].getId() > 0 || playerToDuel.getEquipment().getItems()[duelRule.getEquipmentSlot()].getId() > 0);
        ...
        if (selectedDuelRules[DuelRule.OBSTACLES.ordinal()]) {
            if (selectedDuelRules[DuelRule.NO_MOVEMENT.ordinal()]) {
                Position duelTele = new Position(3366 + RandomUtility.inclusiveRandom(12), 3246 + RandomUtility.inclusiveRandom(6), 0);
            ...
            if (selectedDuelRules[DuelRule.NO_MOVEMENT.ordinal()]) {
                Position duelTele = new Position(3335 + RandomUtility.inclusiveRandom(12), 3246 + RandomUtility.inclusiveRandom(6), 0);
                player.getDueling().duelTelePos = duelTele;
                playerToDuel.getDueling().duelTelePos = player.getDueling().duelTelePos.copy();
                playerToDuel.getDueling().duelTelePos.setX(player.getDueling().duelTelePos.getX() - 1);
            }
    ...
    public void startDuel() {
        ...
        if (selectedDuelRules[DuelRule.OBSTACLES.ordinal()]) {
            if (selectedDuelRules[DuelRule.NO_MOVEMENT.ordinal()]) {
                ...
                player.moveTo(new Position(3366 + RandomUtility.inclusiveRandom(12), 3246 + RandomUtility.inclusiveRandom(6), 0));
            }
        } else {
            if (selectedDuelRules[DuelRule.NO_MOVEMENT.ordinal()]) {
                player.moveTo(duelTelePos);
            } else {
                player.moveTo(new Position(3335 + RandomUtility.inclusiveRandom(12), 3246 + RandomUtility.inclusiveRandom(6), 0));
            }
        }
        player.restart();
    ...
    public void duelVictory() {
        ...
        player.getSession().clearMessages();
        player.moveTo(new Position(3368 + RandomUtility.inclusiveRandom(5), 3267 + RandomUtility.inclusiveRandom(3), 0));
        for (Item item : stakedItems) {
            if (item.getId() > 0 && item.getAmount() > 0) {
                player.getInventory().add(item);
    ...
    public static boolean checkDuel(Player playerToDuel, int statusReq) {
        ...
        return playerToDuel.getDueling().duelingStatus == statusReq && !playerToDuel.isBanking() && !playerToDuel.isShopping() && playerToDuel.getConstitution() > 0 && !playerToDuel.isResting() && goodInterfaceId;
    }
