...
public class FightPit {
    ...
    private static void enterGame(Player player) {
        ...
        int teleportToX = MINIGAME_START_POINT_X + RandomUtility.inclusiveRandom(12);
        int teleportToY = MINIGAME_START_POINT_Y + RandomUtility.inclusiveRandom(12);
        if (!player.getMovementQueue().canWalk(player.getPosition().getX() - teleportToX, player.getPosition().getY() - teleportToY)) {
            teleportToX = MINIGAME_START_POINT_X + RandomUtility.inclusiveRandom(3);
            teleportToY = MINIGAME_START_POINT_Y + RandomUtility.inclusiveRandom(3);
        }
        if (l) {
        ...
        for (Player player : playerMap.keySet()) {
            if (player != null) {
                if (getListCount("PLAYING") == 1 && getState(player) != null && getState(player).equals("PLAYING")) {
                    ...
                    if (giveTokkul) {
                        int amount = 400 + (200 * TOTAL_PLAYERS) + RandomUtility.inclusiveRandom(200);
    ...
    public static boolean inFightPits(final Player player) {
        return FightPit.getState(player) != null && FightPit.getState(player).equals("PLAYING");
    }
