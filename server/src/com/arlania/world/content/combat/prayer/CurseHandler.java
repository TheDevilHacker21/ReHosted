...
public class CurseHandler {
    ...
    public static void handleLeech(Character p, int index, int amount, int maxAmount, boolean decrease) {
        ...
        if (target.getLeechedBonuses()[index] > maxAmount) {
            if (decrease)
                target.getLeechedBonuses()[index] -= RandomUtility.inclusiveRandom(amount);
            else
                target.getLeechedBonuses()[index] -= RandomUtility.inclusiveRandom(amount);
    ...
    private enum CurseData {
        ...
        SOUL_SPLIT(92, 3, 32539, 628),
        TURMOIL(95, 3.5, 32541, 629, new PrayerAnimation(new Animation(12565), new Graphic(2226)));
