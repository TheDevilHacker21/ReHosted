...
public class PrayerHandler {
    ...
    private enum PrayerData {
         ...
         */
        private static final HashMap<Integer, PrayerData> prayerData = new HashMap<Integer, PrayerData>();

        /**
         * Contains the PrayerData with their corresponding buttonId.
         */
        private static final HashMap<Integer, PrayerData> actionButton = new HashMap<Integer, PrayerData>();
    ...
    public static void activatePrayer(Player player, final int prayerId) {
        ...
        if (Dueling.checkRule(player, DuelRule.NO_PRAYER)) {
            ...
            return;
        } else if (player.getLocation() == Location.RECIPE_FOR_DISASTER) {
        ...
        if (prayerId == PIETY && player.getSkillManager().getMaxLevel(Skill.DEFENCE) < 65) {
            player.getPacketSender().sendConfig(pd.configId, 0);
            player.getPacketSender().sendMessage("You need a Defence level of at least 65 to use Piety.");
            return;
        }
        if (prayerId == RIGOUR && player.getSkillManager().getMaxLevel(Skill.DEFENCE) < 70) {
            player.getPacketSender().sendConfig(pd.configId, 0);
            player.getPacketSender().sendMessage("You need a Defence level of at least 70 to use Rigour.");
            return;
        }
        if (prayerId == AUGURY && player.getSkillManager().getMaxLevel(Skill.DEFENCE) < 70) {
            player.getPacketSender().sendConfig(pd.configId, 0);
            player.getPacketSender().sendMessage("You need a Defence level of at least 70 to use Augury.");
            return;
        }
        switch (prayerId) {
            case THICK_SKIN:
