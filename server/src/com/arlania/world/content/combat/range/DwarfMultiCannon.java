...
public class DwarfMultiCannon {
    ...
    public static boolean canSetupCannon(Player c) {
        ...
        return !c.isSettingUpCannon() && c.getConstitution() > 0 && c.getConstitution() > 0;
    }
    ...
    public static void startFiringCannon(Player c, DwarfCannon cannon) {
        if (cannon.cannonFiring() && cannon.getCannonballs() > 99) {
            c.getPacketSender().sendMessage("Your cannon is already full of cannonballs.");
            return;
        }
        if (cannon.getCannonballs() < 100) {
            int playerCannonballs = c.getInventory().getAmount(2) > 100 ? 100 : c.getInventory().getAmount(2);
    ...
    public static void attack(Player player) { //TODO: Bad hardcoding of setting damage, switch to combathittask
        ...
        if (n == null)
            return;
        Hit dmg = new Hit(RandomUtility.inclusiveRandom(300) - RandomUtility.inclusiveRandom(n.getDefinition().getDefenceRange()), Hitmask.RED, CombatIcon.CANNON);
        ...
        player.getAchievementTracker().progress(AchievementData.DEAL_1000_RANGED_DMG, dmg.getDamage());
        player.getAchievementTracker().progress(AchievementData.DEAL_100K_RANGED_DMG, dmg.getDamage());
        player.getAchievementTracker().progress(AchievementData.DEAL_1M_RANGED_DMG, dmg.getDamage());
        player.getAchievementTracker().progress(AchievementData.DEAL_10M_RANGED_DMG, dmg.getDamage());
