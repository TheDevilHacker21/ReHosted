...
public class SummoningTab {
    ...
    public static void handleSpecialAttack(Player c) {
		...
		if(fam == Summoning.FamiliarData.SPIRIT_WOLF) {
			...
			enemy.setDamage(new Damage(new Hit(RandomUtility.getRandom(40), CombatIcon.MAGIC, Hitmask.RED)));
		...
		if(fam == Summoning.FamiliarData.DREADFOWL) {
			...
			enemy.setDamage(new Damage(new Hit(RandomUtility.getRandom(30), CombatIcon.MAGIC, Hitmask.RED)));
		...
		if(fam == Summoning.FamiliarData.SPIRIT_SPIDER) {
			...
			if(RandomUtility.getRandom(2) == 1) {
			...
			} else if(RandomUtility.getRandom(3) == 2) {
		...
		if(fam.name().contains("TITAN")) {
			...
			enemy.setDamage(new Damage(new Hit(RandomUtility.getRandom(250), CombatIcon.MAGIC, Hitmask.CRITICAL)));
			didNothing = false;
		}
		if(didNothing) {
