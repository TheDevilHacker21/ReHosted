...
import com.arlania.world.content.skill.impl.construction.ConstructionData.*;
...
public class Construction {
    ...
    public static void moveHouse(Player p, HouseLocation location) {
        ...
        if (p.getHouseLocation() == location) {
            ...
            if (p.getInventory().getAmount(995) >= location.getCost()) {
                ...
                p.getPacketSender().sendMessage("Your house has succesfully been moved to " + location + ".");
                p.getPacketSender().sendInterfaceRemoval();
            } else {
    ...
    public static void switchTheme(Player p, HouseTheme theme) {
        ...
        if (p.getHouseTheme() == theme) {
            ...
            if (p.getInventory().getAmount(995) >= theme.getCost()) {
                ...
                p.getPacketSender().sendMessage("Your house has succesfully been redecorated as " + theme + ".");
                for (int x = 0; x < 13; x++)
                    for (int y = 0; y < 13; y++)
    ...
    public static void placeAllFurniture(Player p, int heightLevel) {
        Player owner = p.getRegionInstance().getOwner();
        for (HouseFurniture pf : owner.getHouseFurniture()) {
            boolean remove = pf.getRoomZ() != heightLevel;
            Room room = owner.getHouseRooms()[pf.getRoomZ()][pf.getRoomX()][pf.getRoomY()];
            if (room == null)
                return;
            HotSpots hs = HotSpots.forHotSpotIdAndCoords(pf.getHotSpotId(), pf.getStandardXOff(), pf.getStandardYOff(), room);
    ...
    public static String hasReqs(Player p, Furniture f, HotSpots hs) {
        ...
        if (p.getSkillManager().getCurrentLevel(Skill.SKILLER) < f.getLevel()) {
        ...
        if (f.getAdditionalSkillRequirements() != null) {
            for (int ii = 0; ii < f.getAdditionalSkillRequirements().length; ii++) {
                if (p.getSkillManager().getCurrentLevel(Skill.forId(f.getAdditionalSkillRequirements()[ii][0])) < f.getAdditionalSkillRequirements()[ii][1]) {
                    return "You need a " + Skill.forId(f.getAdditionalSkillRequirements()[ii][0]).getFormatName() + " of at least " + f.getAdditionalSkillRequirements()[ii][1] + " to build this.";
                }
            }
        }
    ...
    public static boolean buildActions(Player p, Furniture f, HotSpots hs) {
        ...
        p.getSkillManager().addExperience(Skill.SKILLER, f.getXP());
        ...
        switch (buttonId) {
            ...
            case 2461:
                ...
                if (p.getDialogueActionId() == 444) {
                    ...
                    if (p.getSkillManager().getCurrentLevel(Skill.SKILLER) < b.getConsLevel()) {
    ...
    public static boolean roomExists(Player p) {
        ...
        if (room == null)
            return false;
        return room.getType() != ConstructionData.BUILDABLE && room.getType() != ConstructionData.EMPTY && room.getType() != ConstructionData.DUNGEON_EMPTY;
    }
    ...
    public static void createRoom(int roomType, Player p, int toHeight) {
        ...
        if (rd.getLevelToBuild() > p.getSkillManager().getCurrentLevel(Skill.SKILLER)) {
