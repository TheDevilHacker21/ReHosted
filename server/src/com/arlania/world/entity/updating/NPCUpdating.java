    ...
    public static void update(Player player) {
        ...
        if(player.aggressorTime > 0) {
            for (NPC npc : World.getNpcs()) {
                if (player.getLocalNpcs().size() >= 79) //Originally 255
                    break;
                if (npc == null || !npc.isVisible() || npc.isNeedsPlacement())
                    continue;
                if (npc.getPosition().isWithinDistance(player.getPosition(), player.aggressorRadius)) {
                    player.getLocalAggroNpcs().add(npc);
                }
            }
        }
